...
abstract class AbstractSparkInterpreter extends Interpreter with Logging {
  ...
  protected def postStart(jobContext: JobContext): Unit = {
    if (isSparkSessionPresent()) {
      bind("spark",
        jobContext.sparkSession().getClass.getCanonicalName,
        jobContext.sparkSession(),
        List("""@transient"""))
      bind("sc", "org.apache.spark.SparkContext", jobContext.sc().sc, List("""@transient"""))
      ...
      bind("sc", "org.apache.spark.SparkContext", jobContext.sc().sc, List("""@transient"""))
      val sqlContext = Option(jobContext.hivectx()).getOrElse(jobContext.sqlctx())
