...
object SparkRInterpreter {
  ...
  def apply(conf: SparkConf, jobCtx: JobContext): SparkRInterpreter = {
    jobContext = jobCtx
  ...
  def getSQLContext(): SQLContext = {
    require(jobContext != null)
    if (jobContext.hivectx() != null) jobContext.hivectx() else jobContext.sqlctx()
  }
}
